...
from modulo_hora_atual import mostrar_horario
...
def criar_arquivos(nome_arquivo):
    if not os.path.exists(nome_arquivo):
        arquivo = open(nome_arquivo, 'w', encoding='utf-8').close()
    return


def carregar_placar(nome_arquivo='jogadores.txt', simbolo=';'):
    dic = {}
    criar_arquivos(nome_arquivo)
...
while True:
    ...
    if erros == 6:
        ...
        break
fim = time.time()
m_fim = mostrar_horario()

print('Inicio da partida:', m_inicio)
print('Fim da partida em:', m_fim)

print('Duração da partida %.0f segundos.' % (fim - inicio))